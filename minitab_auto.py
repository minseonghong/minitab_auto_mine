...
def plot_save(series, title, LSL, USL, avg, cpk,min, max, out_png):
    sns.set(style="whitegrid")
    plt.figure(figsize=(10,6))
    
    # 히스토그램
    sns.histplot(series, bins=19, kde=True, stat="density",
    ...
    # 평균
    if avg is not None: 
        plt.axvline(avg, color='blue', ls='-', label=f"Avg={pretty(avg)}")
    ...
    if min is not None: 
        plt.axhline(y=0, color='black', alpha=0, label=f"min={pretty(min,3)}")
        
    if max is not None: 
        plt.axhline(y=0, color='black', alpha=0, label=f"max={pretty(max,3)}")
...
# def plot_save(series, title, LSL, USL, out_png):
#     sns.set(style="whitegrid")
#     plt.figure(figsize=(10,6))
#     sns.histplot(series, bins=19, kde=True, stat="density",
#                  color="black", edgecolor="black")
#     if LSL is not None: plt.axvline(LSL,color='red',ls='--',label=f"LSL={LSL}")
#     if USL is not None: plt.axvline(USL,color='red',ls='--',label=f"USL={USL}")
